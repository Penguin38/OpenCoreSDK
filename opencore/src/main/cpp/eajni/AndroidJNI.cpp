...
AndroidJNI::createJavaThread(const char *name, void (*start)(void *), void *arg) {
    ...
    javaCreateThreadEtc((android_thread_func_t) start, arg, name, ANDROID_PRIORITY_DEFAULT, 0,
            PTHREAD_CREATE_DETACHED, &thread_id);
    return thread_id;
}

android_thread_id_t
AndroidJNI::createJavaThread(const char *name, void (*start)(void *), void *arg, bool canwait) {
    android_thread_id_t thread_id = 0;
    javaCreateThreadEtc((android_thread_func_t) start, arg, name, ANDROID_PRIORITY_DEFAULT, 0,
            canwait? PTHREAD_CREATE_JOINABLE : PTHREAD_CREATE_DETACHED, &thread_id);
    return thread_id;
}
} // namespace android
